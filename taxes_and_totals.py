...
class calculate_taxes_and_totals(object):
	...
	def determine_exclusive_rate(self):
		...
		for item in self.doc.get("items"):
			...
			for i, tax in enumerate(self.doc.get("taxes")):
				...
				# ---------------------------------------------------
				if tax.charge_type == "On Quantity":
					if tax.account_head in item_tax_map:
						if item_tax_map[tax.account_head] == 0:
							tax.tax_amount_for_current_item = tax.rate

							if tax.tax_amount_for_current_item and not self.discount_amount_applied and item.qty:
								item.net_amount = flt(((item.rate -tax.tax_amount_for_current_item)*item.qty), item.precision("net_amount"))
								item.net_rate = flt((item.rate -tax.tax_amount_for_current_item) , item.precision("net_rate"))
								# cumulated_tax_amount += item.net_rate 
								# item.discount_percentage = flt(item.discount_percentage, item.precision("discount_percentage"))

								self._set_in_company_currency(item, ["net_rate", "net_amount"])
						else:	
							tax.tax_amount_for_current_item = item_tax_map[tax.account_head]
	...
	def calculate_net_total(self):
		...
		for item in self.doc.get("items"):
			...
			print "\n\n-----calculate net total-----",self.doc.net_total
			print "\n\n-----calculate net total---net_amount--",item.net_amount
	...
	def calculate_taxes(self):
		...
		# self.tax_breakdown_quantity_section = []
		...
		for n, item in enumerate(self.doc.get("items")):
			...
			for i, tax in enumerate(self.doc.get("taxes")):
				...
				if tax.charge_type == "On Quantity" and cint(tax.included_in_print_rate) == 1 :
					amountt = self.calculate_on_qty_amt(tax, amountt)

		# **********************************************************
		for n, item in enumerate(self.doc.get("items")):
			if tax.charge_type == "On Quantity" and cint(tax.included_in_print_rate) == 1 :
				tax.tax_amount = tax.base_tax_amount = amountt
				# tax.total = tax.base_total - amountt
				tax.total = tax.base_total
				
	def calculate_on_qty_amt(self, tax, amountt):
		amountt = 0.0
		for i, amt in enumerate(self.doc.get("tax_breakdown_quantity")) :
			amountt += amt.total
		return amountt
	...
	def get_current_tax_amount(self, item, tax, item_tax_map):
		...
		# self.doc.tax_breakdown_quantity_section = []
		if tax.charge_type == "Actual":
			...
			if self.doc.doctype == 'Delivery Note' or self.doc.doctype == 'Sales Invoice':
				# tax_rate = 0
				item_doc = frappe.get_doc("Item", item.item_code)
				if cint(tax.included_in_print_rate) == 0 :
					if item_doc.taxes:
						for tax_row in item_doc.taxes:
							if tax_row.tax_type == tax.account_head:
								tax_rate = tax_row.tax_rate
						if tax_rate:
							computed = item.qty * tax_rate
							current_tax_amount += computed
						else:
							tax_rate = tax.rate
							computed = item.qty * tax_rate
							current_tax_amount += computed
						self.tax_breakdown_qty(item, tax_rate, computed, current_tax_amount)
					else:
						computed = item.qty * tax.rate
						current_tax_amount += computed
						self.tax_breakdown_qty(item, tax_rate, computed, current_tax_amount)
				else :
					if item_doc.taxes:
				...
				# 	nl = self.doc.append('tax_breakdown_quantity', {})
				# 	nl.item_code = item.item_code
				# 	nl.qty = item.qty
				# 	nl.tax_rate = tax_rate
				# 	nl.total = computed
				# else:
				# 	computed = item.qty * tax.rate
				# 	current_tax_amount += computed
	...
	def tax_breakdown_qty(self,item, tax_rate, computed, current_tax_amount):
		nl = self.doc.append('tax_breakdown_quantity', {})
		nl.item_code = item.item_code
		nl.qty = item.qty
		nl.tax_rate = tax_rate
	...
	def manipulate_grand_total_for_inclusive_tax(self):
		...
		# if self.doc.get("taxes") and (cint(t.included_in_print_rate) for t in self.doc.get("taxes")):
